...
extern boolean g_demoAvailable; // JulioMOD
...
void D_Display(void)
{
    ...
    case GS_LEVEL:
        ...
        if (automapactive)
            AM_Drawer();
        if ((wipe || (viewheight != 200 && fullscreen)) || (inhelpscreensstate && !inhelpscreens)) // just put away the help screen
            redrawsbar = true;
...
void IdentifyVersion(void)
{
    ...
    {
        gamemode = shareware;
        // gamemission = doom; // Default JulioMOD
        devparm = true;
    ...
    if (!access(doom2wad, R_OK))
    {
        gamemode    = commercial;
        gamemission = doom2; // JulioMOD
        printf("Game mode commercial (Doom II).\n");
        D_AddFile(doom2wad);
        return;
    }
    ...
    {
        gamemode    = commercial;
        gamemission = pack_plut; // JulioMOD
        printf("Game mode commercial (Plutonia).\n");
    ...
    {
        gamemode    = commercial;
        gamemission = pack_tnt; // JulioMOD
        printf("Game mode commercial (TNT).\n");
        D_AddFile(tntwad);
        return;
    }
...
void D_DoomMain(void)
{
    ...
    case commercial:
        ...
        case pack_plut:
            sprintf(title,
                    "                   "
                    "Doom II: Plutonia Experiment v%i.%i"
                    "                           ",
                    VERSION / 100, VERSION % 100);
            break;
        case pack_tnt:
