...
uintptr_t G_CmdChecksum(ticcmd_t *cmd)
{
    size_t i;
    uintptr_t sum = 0;
    
    for (i = 0; i < sizeof(*cmd) / sizeof(ticcmd_t *) - 1; i++)
        sum += ((uintptr_t *)cmd)[i];
...
void G_DoLoadLevel(void)
{
    ...
    if (gamemode == commercial) // JulioMOD
    {
...
void G_DoCompleted(void)
{
    ...
    if (gamemode != commercial)
    {
        if (gamemap == 8)
        {
            ...
            for (i = 0; i < MAXPLAYERS; i++)
                players[i].didsecret = true; // exit secret level
    }
...
void G_InitNew(skill_t skill, int episode, int map)
{
    ...
    // BUG HERE !! 
    /*  JulioRats 05/29/2024
    If you have to load a new game or vice versa, 
        alternately switching between nigthmare and another,
